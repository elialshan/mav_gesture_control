...
{	
		...
		FarnebackPolyExp( const cv::Mat& src, cv::Mat& dst, int n, double sigma )
	{
		...
		cv::Mat_<double> G = cv::Mat_<double>::zeros(6, 6);
		...
		for( y = -n; y <= n; y++ )
			...
			for( y = 0; y < height; y++ )
			{
				...
				for( k = 1; k <= n; k++ )
				{
					...
					srow0 = (float*)(src.data + src.step*cv::max(y-k,0));
					srow1 = (float*)(src.data + src.step*cv::min(y+k,height-1));
	...
	static void
		FarnebackUpdateFlow_GaussianBlur( const cv::Mat& _R0, const cv::Mat& _R1,
		...
		bool update_matrices )
	{
		...
		int m = block_size/2;
		int y0 = 0, y1;
		int min_update_stripe = cv::max((1 << 10)/width, block_size);
		double sigma = m*0.3, s = 1;

		cv::AutoBuffer<float> _vsum((width+m*2+2)*5 + 16), _hsum(width*5 + 16);
		cv::AutoBuffer<float, 4096> _kernel((m+1)*5 + 16);
		cv::AutoBuffer<float*, 1024> _srow(m*2+1);
		float *vsum = cv::alignPtr((float*)_vsum + (m+1)*5, 16), *hsum = cv::alignPtr((float*)_hsum, 16);
...
#if CV_SSE2
		float* simd_kernel = cv::alignPtr(kernel + m+1, 16);
		volatile bool useSIMD = cv::checkHardwareSupport(CV_CPU_SSE);
		if( useSIMD )
		{                    
			for( i = 0; i <= m; i++ )
		...
		for( y = 0; y < height; y++ )
		{
			...
			for( i = 0; i <= m; i++ )
			{
				srow[m-i] = (const float*)(matM.data + matM.step*cv::max(y-i,0));
				srow[m+i] = (const float*)(matM.data + matM.step*cv::min(y+i,height-1));
			}
		...
		std::vector<float>& fscales, int poly_n, double poly_sigma)
	{
		...
		for(int k = 0; k < (int)poly_exp_pyr.size(); k++)
		{
	        cv::Mat R, I;
			...
			if (fscales[k] != 1.0f) {
				...
				smooth_sz = cv::max(smooth_sz, 3);			
				GaussianBlur(fimg, fimg, cv::Size(smooth_sz, smooth_sz), sigma, sigma);
				resize(fimg, I, cv::Size(width, height), CV_INTER_LINEAR);
			}
			else
